...
example {z : ℝ} (h1 : z ^ 2 - 2 = 0) : z ^ 4 - z ^ 3 - z ^ 2 + 2 * z + 1 = 3 :=
  calc
    z^4 - z^3 - z^2 + 2 * z + 1 = z * (z^3 -(z^2 - 2) - z) + 1 := by ring
    _ = z * (z^3 - 0 - z) + 1 := by rw[h1]
    _ = z^2 * (z^2 - 2 + 1) + 1 := by ring
    _ = z^2 * (0 + 1) + 1 := by rw[h1]
    _ = z^2 -2 + 3 := by ring
    _ = 0 + 3 := by rw[h1]
    _ = 3 := by ring
...
example {u v : ℝ} (h1 : u + 1 = v) : u ^ 2 + 3 * u + 1 = v ^ 2 + v - 1 :=
  calc
    u^2 + 3 * u + 1 = (u + 1 - 1)^2 + 3 *(u + 1) -2 := by ring
    _ = (v - 1)^2 + 3 * v - 2 := by rw[h1]
    _ = v^2 + v - 1 := by ring
...
example {p q r : ℚ} (h1 : p + q + r = 0) (h2 : p * q + p * r + q * r = 2) :
    p ^ 2 + q ^ 2 + r ^ 2 = -4 :=
  calc
    p ^ 2 + q ^ 2 + r ^ 2 = (p + q)^2 - 2*p*q + r^2 := by ring
